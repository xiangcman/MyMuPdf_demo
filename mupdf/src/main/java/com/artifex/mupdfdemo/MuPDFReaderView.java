...
public class MuPDFReaderView extends ReaderView {
    ...
    public enum Mode { Viewing, Selecting, Drawing }//drawing表示正在绘制，selecting表示正在别选中了，viewing表示正常状态

    private final Context mContext;
    private boolean mLinksEnabled = false;// 超链接是否高亮显示
    private boolean isLinkHighlightColor = false;// 是否设置了超链接颜色
    private Mode mMode = Mode.Viewing;
    private boolean tapDisabled = false;//点击是否无效，true表示不能有点击功能到下一页
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        if (mMode == Mode.Viewing && !tapDisabled) {
            ...
            //取消掉绘制的区域
            Log.d("ReaderView", "mMode:" + mMode.name());
            //mMode = Mode.Viewing;
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        Log.d("ReaderView", "onScroll");
        MuPDFView pageView = (MuPDFView) getDisplayedView();
        switch (mMode) {
            case Viewing://如果是正常状态没有选中文字
                if (!tapDisabled) onDocMotion();//如果可以点击则影藏工具栏

                return super.onScroll(e1, e2, distanceX, distanceY);//这里就是页面上滑动的逻辑
            case Selecting://当有文字被选中的时候，页面上才可以继续选中文字
    ...
    public boolean onScaleBegin(ScaleGestureDetector d) {
        ...
        Log.d("ReaderView", "onScaleBegin");
        tapDisabled = true;//放大缩小的时候不让翻页
